...
float Input::getAxis(Axis axis) {
    switch(axis) {
        ...
        case Axis::Elevation:
            return (isKeyPressed(GLFW_KEY_E) ? 1.0f : 0.0f) - (isKeyPressed(GLFW_KEY_Q) ? 1.0f : 0.0f);
        ...
        case Axis::LookVertical:
            return (isKeyPressed(GLFW_KEY_DOWN) ? 1.0f : 0.0f) - (isKeyPressed(GLFW_KEY_UP) ? 1.0f : 0.0f);
        default:
            return 0.0f;
    }
}
